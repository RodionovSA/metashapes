...
## Corpus Check
- 77 files · ~47,829 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1637 nodes · 5153 edges · 114 communities (61 shown, 53 thin omitted)
- Extraction: 75% EXTRACTED · 25% INFERRED · 0% AMBIGUOUS · INFERRED: 1288 edges (avg confidence: 0.61)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9bceb448`
...
## Communities (114 total, 53 thin omitted)
...
Cohesion: 0.05
Nodes (55): Constraint-Based Unit Cell Generation, RandomUnitCellGenerator._sample_shape, register_shape_sampler(), SHAPE_SAMPLER_REGISTRY, Lattice, Cartesian translation for lattice cell (i, j)., Cartesian (x, y) -> fractional (f1, f2)., Fractional (f1, f2) -> Cartesian (x, y). (+47 more)
...
Cohesion: 0.16
Nodes (17): TestBarSafeExtraction, TestCrossSafeExtraction, TestIsoscelesTrapezoidSafeExtraction, TestRegularPolygonSafeExtraction, TestStadiumSafeExtraction, TestStripeSafeExtraction, TestUnitCellSafeExtraction, from_parametric() (+9 more)
...
Cohesion: 0.08
Nodes (8): Bar, min_feature_size(), _project(), An infinite bar spanning the full unit cell along one axis.      The bar is unbo, An infinite bar spanning the full unit cell along one axis.      The bar is unbo, BarSDF(), TestBar, TestBarDtypeDeviceGrad
...
Cohesion: 0.06
Nodes (23): _floor_vector_norm(), IsoscelesTrapezoid, _max_corner_radius(), _project(), _quad_vertices(), Symbolic isosceles trapezoid.      Parameters:         center: (cx, cy), Symbolic isosceles trapezoid.      Parameters:         center: (cx, cy), Symbolic isosceles trapezoid.      Parameters:         center: (cx, cy) (+15 more)

### Community 16 - "PyTorch Differentiability"
Cohesion: 0.10
Nodes (13): Parametric Serialization (to_parametric / from_parametric), Base class for all symbolic 2D shapes., Base class for all symbolic 2D shapes., Snap this shape's constrained parameters back into their valid         ranges, i, Snap this shape's constrained parameters back into their valid         ranges, i, Shape, SHAPE_REGISTRY Dict, min_feature_size() (+5 more)

### Community 17 - "Lattice SDF Tests"
Cohesion: 0.11
Nodes (8): ConvexQuad, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, TestConvexQuad, TestRectangle
...
Cohesion: 0.12
Nodes (19): SDF Convention: Negative Inside, Positive Outside, assert_bounds_contain(), assert_direct_call_dtype_promotion(), assert_dtype_device_flow(), assert_gradients_finite(), _assert_param_grads_finite(), assert_round_trip(), Calling sdf() directly on a primitive (bypassing UnitCell) with     float64 quer (+11 more)

### Community 29 - "Adapters Test Init"
Cohesion: 0.15
Nodes (12): min_feature_size(), min_feature_size(), min_feature_size(), min_feature_size(), from_parametric(), register_shape(), from_parametric(), Register `value` on `module` under `name`.      If `value` is an nn.Parameter it (+4 more)
...
Cohesion: 0.15
Nodes (23): CellMetrics, Smooth Boolean Operations via Polynomial Blending, CellMetrics, Per-cell measurements produced by :class:`UnitCellAnalyzer`.      Attributes, Difference, Intersection, _project(), Symbolic difference of two shapes: left - right. (+15 more)
...
Cohesion: 0.21
Nodes (4): assert_inside(), assert_outside(), All points should have SDF < tol (inside or on boundary)., All points should have SDF > -tol (outside or on boundary).

### Community 61 - "Community 61"
Cohesion: 0.10
Nodes (8): center_scene(method='centroid') must work even if params have grad., Verifies that calling shape_to_shapely (which does .detach().cpu())         does, TestTransformsToShapely, TestOffsetSearchOverhead, Symbolic rectangle.      Parameters:         center: (cx, cy)         size: (wid, Symbolic rectangle.      Parameters:         center: (cx, cy)         size: (wid, Rectangle, TestRectangleDtypeDeviceGrad
...
Cohesion: 0.09
Nodes (11): Ellipse, Ellipse.min_feature_size, Symbolic ellipse.      Parameters:         center: (cx, cy)         axes: full s, Symbolic ellipse.      Parameters:         center: (cx, cy)         axes: full s, Symbolic ellipse.      Parameters:         center: (cx, cy)         axes: full s, Symbolic ellipse.      Parameters:         center: (cx, cy)         axes: full s, assert_gradients_finite_at(), Same as assert_gradients_finite but evaluates sdf() at a single,     possibly su (+3 more)
...
Cohesion: 0.09
Nodes (12): square_in_rect(), method='centroid' on a shape that Shapely can represent should work., TestInfiniteBoundsError, TestOffsetsForRing, TestToShapely, TestUnitCellSerialization, A periodic structure: one Lattice + one Shape (the scene).      The lattice owns, Return a new UnitCell with the scene translated to the cell centre.          The (+4 more)
...
- **Why does `Shape` connect `PyTorch Differentiability` to `Shape Primitives Core`, `Random Generator & Lattice`, `Shape Analysis & SDF Concepts`, `Unit Cell Analyzer`, `Conic Shape Primitives`, `Shapely Transform Tests`, `Shapely Compound Shape Tests`, `Generator Integration Tests`, `Generator Base Classes`, `Community 11`, `Shapely Adapter Modules`, `Unit Cell Tests`, `Shapely Adapter Layer`, `Lattice SDF Tests`, `Coordinate Transform Bridge`, `Periodic Unit Cell Ops`, `Shape Test Init`, `Lattice Test Init`, `Adapters Test Init`, `Lattice Basis Rationale`, `Lattice Basis Rationale B`, `Validator Rationale`, `Generator Base Rationale`, `Community 52`, `Community 61`, `Community 69`, `Community 71`, `Community 77`, `Community 80`, `Community 83`?**
  _High betweenness centrality (0.136) - this node is a cross-community bridge._
- **Why does `Rectangle` connect `Community 61` to `Random Generator & Lattice`, `Shape Analysis & SDF Concepts`, `Unit Cell Analyzer`, `YAML Serialization Tests`, `Shapely Transform Tests`, `Random Generator Logic`, `Shapely Compound Shape Tests`, `Generator Integration Tests`, `Shapely Adapter Modules`, `YAML & Unit Cell Serialization`, `Shapely Adapter Layer`, `PyTorch Differentiability`, `Lattice SDF Tests`, `Boolean Shape Tests`, `Community 24`, `Community 25`, `Shape Test Init`, `Adapters Test Init`, `Lattice Basis Rationale`, `Generator Base Rationale`, `Community 52`, `Community 53`, `Community 55`, `Community 65`, `Community 70`, `Community 75`, `Community 83`?**
  _High betweenness centrality (0.113) - this node is a cross-community bridge._
- **Why does `UnitCell` connect `Community 75` to `Shape Primitives Core`, `Shape Analysis & SDF Concepts`, `Unit Cell Analyzer`, `Generator Base Rationale`, `YAML Serialization Tests`, `Community 70`, `Shapely Transform Tests`, `Shapely Compound Shape Tests`, `Random Generator Logic`, `Community 11`, `YAML & Unit Cell Serialization`, `PyTorch Differentiability`, `Boolean Shape Tests`, `Community 52`, `Community 53`, `Community 24`, `Community 61`, `Lattice Basis Rationale`?**
