...
## Corpus Check
- 74 files · ~50,240 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1265 nodes · 4244 edges · 65 communities (45 shown, 20 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 935 edges (avg confidence: 0.57)
...
Cohesion: 0.20
Nodes (19): CellMetrics, _bbox_size(), CellMetrics, Per-cell measurements produced by :class:`UnitCellAnalyzer`.      Attributes, _to_geom(), Symbolic union of two shapes., Union, Symbolic scaling of a shape. (+11 more)

### Community 1 - "Random Generator & Lattice"
Cohesion: 0.06
Nodes (35): _centroid(), TestBooleansToShapely, TestPrimitivesToShapely, TestTransformsToShapely, Shapely Adapter Pattern, difference_to_shapely(), intersection_to_shapely(), union_to_shapely() (+27 more)
...
Cohesion: 0.13
Nodes (40): ABC, Constraint-Based Unit Cell Generation, RandomUnitCellGenerator._sample_shape, register_shape_sampler(), SHAPE_SAMPLER_REGISTRY, Lattice, Cartesian translation for lattice cell (i, j)., In-plane periodicity of the unit cell. Fixed (non-optimizable).     Defined by t (+32 more)

### Community 4 - "Conic Shape Primitives"
Cohesion: 0.07
Nodes (19): SDF Convention: Negative Inside, Positive Outside, Egg, Ellipse, _ellipse_closest_point(), Ellipse.min_feature_size, Symbolic ellipse.      Parameters:         center: (cx, cy)         axes: full s, Symbolic ellipse.      Parameters:         center: (cx, cy)         axes: full s, Egg shape: two half-ellipses joined at the x-axis.      Parameters:         cent (+11 more)
...
Cohesion: 0.12
Nodes (10): Cross, Symbolic T-shape.      Parameters:         center: (cx, cy)         length: full, Symbolic T-shape.      Parameters:         center: (cx, cy)         length: full, Symbolic symmetric cross.      Parameters:         center: (cx, cy)         leng, TShape, Shape, assert_inside(), All points should have SDF < tol (inside or on boundary). (+2 more)
...
Cohesion: 0.06
Nodes (6): TestLatticeConstruction, TestLatticeCoordinates, TestLatticeOffset, TestLatticePackageExports, TestLatticeProperties, TestNeighborOffsets
...
Cohesion: 0.08
Nodes (20): p(), pv(), center_scene(method='centroid') must work even if params have grad., Verifies that calling shape_to_shapely (which does .detach().cpu())         does, Shorthand: make a scalar nn.Parameter., Shorthand: make a vector nn.Parameter., TestBarSafeExtraction, TestCrossSafeExtraction (+12 more)
...
Cohesion: 0.10
Nodes (9): TestCompoundShapesToShapely, ConvexQuad, IsoscelesTrapezoid, Symbolic rectangle.      Parameters:         center: (cx, cy)         size: (wid, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, Rectangle, TestConvexQuad (+1 more)

### Community 18 - "Coordinate Transform Bridge"
Cohesion: 0.14
Nodes (4): Regular n-pointed star.      Parameters:         center: (cx, cy)         n: num, Regular n-pointed star.      Parameters:         center: (cx, cy)         n: num, Star, TestStar
...
Cohesion: 0.16
Nodes (7): Cross and TShape used to each carry their own inline copy of the     rounded-box, TestSharedRoundedBoxHelper, Direct tests of the shared helper, plus a check that Rectangle's own     sdf() a, TestSdfRoundedBox, SDF of an axis-aligned box of half-extents (hx, hy) with corners     rounded by, _sdf_rounded_box(), _to_local_coords()

### Community 28 - "Lattice Test Init"
Cohesion: 0.38
Nodes (4): assert_bounds_contain(), assert_round_trip(), Serialise → deserialise and verify SDF equality on a small grid., All points should lie inside (or on) the reported bounding box.
...
Cohesion: 0.20
Nodes (8): _compute_min_gap, Rotation Guard for Infinite-Extent Shapes, _compute_min_gap(), True if this periodic shift should be skipped for self-gap measurement.      A s, Minimum distance between any two shapes (or a shape and its own periodic     ima, _skip_self_shift(), TestComputeMinGap, TestSkipSelfShift
...
- **Are the 91 inferred relationships involving `Rectangle` (e.g. with `Shape` and `TestLeafShapes`) actually correct?**
  _`Rectangle` has 91 INFERRED edges - model-reasoned connections that need verification._
