...
## Corpus Check
- 76 files · ~47,573 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1583 nodes · 5095 edges · 91 communities (64 shown, 27 thin omitted)
- Extraction: 75% EXTRACTED · 25% INFERRED · 0% AMBIGUOUS · INFERRED: 1284 edges (avg confidence: 0.61)
...
Cohesion: 0.06
Nodes (40): _centroid(), TestBooleansToShapely, TestPrimitivesToShapely, _compute_min_gap, _compute_min_gap(), Minimum distance between any two shapes (or a shape and its own periodic     ima, Shapely Adapter Pattern, difference_to_shapely() (+32 more)
...
Cohesion: 0.05
Nodes (54): Constraint-Based Unit Cell Generation, RandomUnitCellGenerator._sample_shape, register_shape_sampler(), SHAPE_SAMPLER_REGISTRY, Lattice, Cartesian translation for lattice cell (i, j)., Cartesian (x, y) -> fractional (f1, f2)., Fractional (f1, f2) -> Cartesian (x, y). (+46 more)
...
Cohesion: 0.08
Nodes (11): Cross, Symbolic T-shape.      Parameters:         center: (cx, cy)         length: full, Symbolic T-shape.      Parameters:         center: (cx, cy)         length: full, Symbolic T-shape.      Parameters:         center: (cx, cy)         length: full, Symbolic symmetric cross.      Parameters:         center: (cx, cy)         leng, TShape, Shape, TestCross (+3 more)

### Community 10 - "Generator Base Classes"
Cohesion: 0.09
Nodes (7): General triangle defined by two base angles and the base length (ASA).      Para, (A, B, C) as (x, y) tensor pairs, CCW, centroid at origin., Triangle, assert_inside(), All points should have SDF < tol (inside or on boundary)., TestTriangle, TestTriangleDtypeDeviceGrad
...
Cohesion: 0.09
Nodes (6): TestUnitCellSerialization, Symbolic regular polygon.      Parameters:         center: (cx, cy)         n: N, Symbolic regular polygon.      Parameters:         center: (cx, cy)         n: N, RegularPolygon, TestRegularPolygon, TestRegularPolygonDtypeDeviceGrad

### Community 15 - "Shapely Adapter Layer"
Cohesion: 0.12
Nodes (7): IsoscelesTrapezoid, Symbolic isosceles trapezoid.      Parameters:         center: (cx, cy), Symbolic isosceles trapezoid.      Parameters:         center: (cx, cy), Symbolic isosceles trapezoid.      Parameters:         center: (cx, cy), Symbolic isosceles trapezoid.      Parameters:         center: (cx, cy), TestIsoscelesTrapezoid, TestIsoscelesTrapezoidDtypeDeviceGrad

### Community 16 - "PyTorch Differentiability"
Cohesion: 0.10
Nodes (14): square_in_rect(), _brute_force_sdf(), Independent reference: fold into the cell, then search a ring far     larger tha, Independent reference: fold into the cell, then search a ring far     larger tha, Independent reference: fold into the cell, then search a ring far     larger tha, Independent reference: fold into the cell, then search a ring far     larger tha, sdf() after the L-02/L-03 rewrite must agree with a large brute-force     refere, sdf() must agree with a large brute-force reference search for a     variety of (+6 more)

### Community 17 - "Lattice SDF Tests"
Cohesion: 0.12
Nodes (6): ConvexQuad, IsoscelesTrapezoid, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, TestConvexQuad
...
Cohesion: 0.10
Nodes (8): SDF Convention: Negative Inside, Positive Outside, Egg, Egg shape: two half-ellipses joined at the x-axis.      Parameters:         cent, sdf_at(), test_matches_brute_force_nearest_point(), test_matches_brute_force_nearest_point(), TestEgg, TestEggDtypeDeviceGrad
...
Cohesion: 0.15
Nodes (17): assert_bounds_contain(), assert_direct_call_dtype_promotion(), assert_dtype_device_flow(), assert_gradients_finite(), assert_outside(), _assert_param_grads_finite(), assert_round_trip(), Calling sdf() directly on a primitive (bypassing UnitCell) with     float64 quer (+9 more)

### Community 29 - "Adapters Test Init"
Cohesion: 0.15
Nodes (9): Cross and TShape used to each carry their own inline copy of the     rounded-box, Cross and TShape both call shape/utils.py's _sdf_rounded_box. These     checks p, Cross and TShape used to each carry their own inline copy of the     rounded-box, TestSharedRoundedBoxHelper, Direct tests of the shared helper, plus a check that Rectangle's own     sdf() a, Direct tests of the shared helper, plus a check that Rectangle's own     sdf() a, TestSdfRoundedBox, SDF of an axis-aligned box of half-extents (hx, hy) with corners     rounded by (+1 more)
...
Cohesion: 0.15
Nodes (9): min_feature_size(), min_feature_size(), min_feature_size(), register_shape(), Register `value` on `module` under `name`.      If `value` is an nn.Parameter it, Register `value` on `module` under `name`.      If `value` is an nn.Parameter it, Register `value` on `module` under `name`.      If `value` is an nn.Parameter it, register() (+1 more)
...
Cohesion: 0.19
Nodes (6): Rotation Guard for Infinite-Extent Shapes, _bbox_size(), True if this periodic shift should be skipped for self-gap measurement.      A s, _skip_self_shift(), _to_geom(), TestSkipSelfShift

### Community 61 - "Community 61"
Cohesion: 0.14
Nodes (7): TestTransformsToShapely, TestOffsetSearchOverhead, Symbolic rectangle.      Parameters:         center: (cx, cy)         size: (wid, Symbolic rectangle.      Parameters:         center: (cx, cy)         size: (wid, Rectangle, TestRectangle, TestRectangleDtypeDeviceGrad
...
Cohesion: 0.18
Nodes (19): CellMetrics, Smooth Boolean Operations via Polynomial Blending, CellMetrics, Per-cell measurements produced by :class:`UnitCellAnalyzer`.      Attributes, Difference, Intersection, Symbolic difference of two shapes: left - right., Symbolic difference of two shapes: left - right. (+11 more)
...
Cohesion: 0.22
Nodes (7): from_parametric(), min_feature_size(), Tensor -> JSON/YAML-serializable Python value.     Scalar tensor -> Python scala, Tensor -> JSON/YAML-serializable Python value.     Scalar tensor -> Python scala, Tensor -> JSON/YAML-serializable Python value.     Scalar tensor -> Python scala, to_plain_data(), from_parametric()
...
Cohesion: 0.25
Nodes (3): is_empty_bounds(), from_parametric(), TestIsEmptyBounds
...
Cohesion: 0.25
Nodes (6): ConvexQuadSDF(), IsoscelesTrapezoidSDF(), SDF for an isosceles trapezoid, symmetric about the y-axis.      The bottom base, SDF for a convex quadrilateral.                   Parameters:              u:, SDF for a rectangle.           Parameters:          hx: Half of the rectangle si, RectangleSDF()
...
- **Why does `Shape` connect `Shape Test Init` to `Shape Primitives Core`, `Random Generator & Lattice`, `Shape Analysis & SDF Concepts`, `Unit Cell Analyzer`, `Conic Shape Primitives`, `Shapely Transform Tests`, `Shapely Compound Shape Tests`, `Generator Integration Tests`, `Generator Base Classes`, `Community 11`, `Shapely Adapter Modules`, `Unit Cell Tests`, `Shapely Adapter Layer`, `Lattice SDF Tests`, `Coordinate Transform Bridge`, `Periodic Unit Cell Ops`, `Lattice Test Init`, `Lattice Basis Rationale`, `Lattice Basis Rationale B`, `Validator Rationale`, `Generator Base Rationale`, `Community 52`, `Community 55`, `Community 61`, `Community 65`, `Community 69`, `Community 70`, `Community 74`, `Community 77`?**
  _High betweenness centrality (0.137) - this node is a cross-community bridge._
- **Why does `Rectangle` connect `Community 61` to `Random Generator & Lattice`, `Shape Analysis & SDF Concepts`, `Unit Cell Analyzer`, `YAML Serialization Tests`, `Shapely Transform Tests`, `Random Generator Logic`, `Shapely Compound Shape Tests`, `Generator Integration Tests`, `Shapely Adapter Modules`, `YAML & Unit Cell Serialization`, `Unit Cell Tests`, `PyTorch Differentiability`, `Lattice SDF Tests`, `Boolean Shape Tests`, `Community 24`, `Community 25`, `Shape Test Init`, `Adapters Test Init`, `Lattice Basis Rationale`, `Community 52`, `Community 53`, `Community 55`, `Community 65`, `Community 69`, `Community 70`, `Community 75`, `Community 76`, `Community 77`, `Community 79`, `Community 91`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Why does `UnitCell` connect `Community 75` to `Shape Primitives Core`, `Shape Analysis & SDF Concepts`, `Unit Cell Analyzer`, `YAML Serialization Tests`, `Shapely Transform Tests`, `Random Generator Logic`, `Shapely Compound Shape Tests`, `Community 11`, `YAML & Unit Cell Serialization`, `Unit Cell Tests`, `PyTorch Differentiability`, `Boolean Shape Tests`, `Community 24`, `Lattice Basis Rationale`, `Community 53`, `Community 55`, `Community 61`, `Community 70`, `Community 76`, `Community 80`?**
