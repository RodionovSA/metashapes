...
## Corpus Check
- 72 files · ~41,537 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1083 nodes · 3897 edges · 58 communities (44 shown, 14 thin omitted)
- Extraction: 79% EXTRACTED · 21% INFERRED · 0% AMBIGUOUS · INFERRED: 811 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5d6fdb63`
...
1. `UnitCell` - 133 edges
2. `Lattice` - 124 edges
3. `Rectangle` - 117 edges
...
- `TestUnitCellSDF` --uses--> `UnitCell`  [INFERRED]
  tests/lattice/test_unit_cell.py → src/metashapes/lattice/unit_cell.py
- `TestUnitCellRasterize` --uses--> `UnitCell`  [INFERRED]
  tests/lattice/test_unit_cell.py → src/metashapes/lattice/unit_cell.py
...
Cohesion: 0.16
Nodes (14): Smooth Boolean Operations via Polynomial Blending, from_parametric(), min_feature_size(), Tensor -> JSON/YAML-serializable Python value.     Scalar tensor -> Python scala, to_plain_data(), Difference, from_parametric(), Intersection (+6 more)
...
Cohesion: 0.08
Nodes (29): _compute_min_gap, _leaf_shapes, UnitCellAnalyzer, UnitCellAnalyzer.validate, Rotation Guard for Infinite-Extent Shapes, _compute_min_gap(), _leaf_shapes(), Compute all metrics for a single cell. (+21 more)
...
Cohesion: 0.06
Nodes (17): Egg, Ellipse, _ellipse_closest_point(), Ellipse.min_feature_size, Symbolic ellipse.      Parameters:         center: (cx, cy)         axes: full s, Symbolic ellipse.      Parameters:         center: (cx, cy)         axes: full s, Egg shape: two half-ellipses joined at the x-axis.      Parameters:         cent, Nearest point on the boundary of an axis-aligned ellipse (semi-axes     a, b, ce (+9 more)
...
Cohesion: 0.17
Nodes (4): ConvexQuad, IsoscelesTrapezoid, Symbolic convex quadrilateral with optional rounded corners.      The quad is bu, TestConvexQuad
...
Cohesion: 0.09
Nodes (18): cell_area(), device(), dtype(), hexagonal(), matrix(), rectangular(), cartesian_grid(), fractional_grid() (+10 more)
...
Cohesion: 0.17
Nodes (6): IsoscelesTrapezoid, _max_corner_radius(), _quad_vertices(), Symbolic isosceles trapezoid.      Parameters:         center: (cx, cy), _signed_area2(), TestIsoscelesTrapezoid
...
Cohesion: 0.29
Nodes (3): TestToShapely, A periodic structure: one Lattice + one Shape (the scene).      The lattice owns, UnitCell
...
Cohesion: 0.19
Nodes (16): TestBarSafeExtraction, TestCrossSafeExtraction, TestEllipseSafeExtraction, TestIsoscelesTrapezoidSafeExtraction, TestRectangleSafeExtraction, TestRegularPolygonSafeExtraction, TestStadiumSafeExtraction, TestStripeSafeExtraction (+8 more)

### Community 52 - "Community 52"
Cohesion: 0.18
Nodes (9): min_feature_size(), min_feature_size(), min_feature_size(), min_feature_size(), register_shape(), from_parametric(), Register `value` on `module` under `name`.      If `value` is an nn.Parameter it, register() (+1 more)

### Community 53 - "Community 53"
Cohesion: 0.15
Nodes (6): Parametric Serialization (to_parametric / from_parametric), Base class for all symbolic 2D shapes., Signed distance evaluated on torch tensors.         x, y can be broadcastable te, Axis-aligned bounding box of the shape, in world coordinates.          Returns (, Shape, SHAPE_REGISTRY Dict

### Community 54 - "Community 54"
Cohesion: 0.30
Nodes (6): CellMetrics, _bbox_size(), CellMetrics, Per-cell measurements produced by :class:`UnitCellAnalyzer`.      Attributes, _to_geom(), TestCellMetrics
...
- **Why does `Lattice` connect `Unit Cell Analyzer` to `Lattice Basis Rationale B`, `Shape Analysis & SDF Concepts`, `Validator Rationale`, `YAML Serialization Tests`, `Random Generator Logic`, `Shapely Compound Shape Tests`, `Generator Integration Tests`, `Community 11`, `YAML & Unit Cell Serialization`, `Lattice SDF Tests`, `Boolean Shape Tests`, `Periodic Unit Cell Ops`, `Community 54`, `Community 24`, `Community 25`, `Adapters Test Init`, `Generators Test Init`, `Lattice Basis Rationale`?**
  _High betweenness centrality (0.149) - this node is a cross-community bridge._
- **Why does `Shape` connect `Community 53` to `Shape Primitives Core`, `Random Generator & Lattice`, `Shape Analysis & SDF Concepts`, `Unit Cell Analyzer`, `Conic Shape Primitives`, `YAML Serialization Tests`, `Shapely Transform Tests`, `Shapely Compound Shape Tests`, `Generator Base Classes`, `Community 11`, `Shapely Adapter Modules`, `Unit Cell Tests`, `PyTorch Differentiability`, `Lattice SDF Tests`, `Coordinate Transform Bridge`, `Periodic Unit Cell Ops`, `Community 23`, `Shape Test Init`, `Lattice Test Init`, `Validator Rationale`, `Community 52`, `Community 54`, `Community 55`?**
  _High betweenness centrality (0.129) - this node is a cross-community bridge._
- **Why does `UnitCell` connect `Generators Test Init` to `Lattice Basis Rationale B`, `Shape Analysis & SDF Concepts`, `Unit Cell Analyzer`, `Validator Rationale`, `YAML Serialization Tests`, `Random Generator Logic`, `Shapely Compound Shape Tests`, `Community 11`, `YAML & Unit Cell Serialization`, `Shapely Adapter Layer`, `Boolean Shape Tests`, `Periodic Unit Cell Ops`, `Community 54`, `Community 23`, `Community 24`, `Community 25`, `Adapters Test Init`, `Lattice Basis Rationale`?**
  _High betweenness centrality (0.120) - this node is a cross-community bridge._
- **Are the 66 inferred relationships involving `UnitCell` (e.g. with `TestUnitCellSDF` and `TestUnitCellRasterize`) actually correct?**
  _`UnitCell` has 66 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 71 inferred relationships involving `Rectangle` (e.g. with `TestUnitCellSDF` and `TestUnitCellRasterize`) actually correct?**
  _`Rectangle` has 71 INFERRED edges - model-reasoned connections that need verification._
